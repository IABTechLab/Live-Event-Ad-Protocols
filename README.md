...
#### Versioning Policy
The version numbers of these APIs will only be incremented on breaking changes. In other words, APIs with minor version changes should be considered a distinct version from the origninal when there is a need for distinguishing versions. For example, a demand source might regard the version header when parsing a bid request received from a supply source. See OpenRTB Principles.
...
The format for version numbering includes major and minor version and a date code. For example, 1.0-202507 represents the release for July 2025. The following releases may be 1.0-202509 (September), 1.0-202512 (December), etc.
...
Over the course of each month, the Programmatic Supply Chain Working Group and Commit Group may review any submitted Pull Requests and take the following possible actions for each:
- approve it for inclusion in the next release
- ask the author(s) for additional changes
- reject it (with a rationale)

During the last week of the month, if there are any approved Pull Requests in the `develop` branch, the following steps are executed:

1. A PR is created to merge the `develop` branch into the `main` branch.
1. A new Release and Tag are created concurrently. The naming convention for the release is "API major.minor-YYYYMM", and the tag is "major.minor-YYYYMM" where YYYYMM is the date code (e.g. 202301 for January 2023).
...
Live Event Ad Program, a sub-Working Group of Programmatic Supply Chain Working Group members provide contributions to this repository. Participants in the LIve Event Ad Program and/or the Programmatic Supply Chain Working Group(s) must be members of IAB Tech Lab. Technical Governance and code commits for the project are provided by the IAB Tech Lab Programmatic Supply Chain Commit Group. 
